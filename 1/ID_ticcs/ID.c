...
void ID_step(void)
{
   ...
   */
  rtb_Sum1 = cnt1 - ID_P.Constant1_Value;
   ...
   *  Sum: '<S1>/Sum1'
   */
  rtb_Sum1 = floor(rtb_Sum1);
  if (rtIsNaN(rtb_Sum1) || rtIsInf(rtb_Sum1)) {
    rtb_Sum1 = 0.0;
  } else {
    rtb_Sum1 = fmod(rtb_Sum1, 65536.0);
  }

  ID_B.DataTypeConversion1[0] = rtb_Sum1 < 0.0 ? (uint16_T)-(int16_T)(uint16_T)
    -rtb_Sum1 : (uint16_T)rtb_Sum1;
  rtb_Sum1 = floor(cnt2 - ID_P.Constant1_Value_g);
  if (rtIsNaN(rtb_Sum1) || rtIsInf(rtb_Sum1)) {
    rtb_Sum1 = 0.0;
  } else {
    rtb_Sum1 = fmod(rtb_Sum1, 65536.0);
  ...
  ID_B.DataTypeConversion1[1] = rtb_Sum1 < 0.0 ? (uint16_T)-(int16_T)(uint16_T)
    -rtb_Sum1 : (uint16_T)rtb_Sum1;
  ...
  {
    char txString[4];
    byteswap_L8exp(txString, (char *)ID_B.DataTypeConversion1, 2, 2);
   ...
   */
  if (ID_P.Constant_Value >= ID_P.Saturation_UpperSat) {
    rtb_Sum1 = ID_P.Saturation_UpperSat;
  } else if (ID_P.Constant_Value <= ID_P.Saturation_LowerSat) {
    rtb_Sum1 = ID_P.Saturation_LowerSat;
  } else {
    rtb_Sum1 = ID_P.Constant_Value;
  }
   ...
   *  Sum: '<S3>/Sum2'
   */
  rtb_Sum1 = ID_P.DAOutput_Gain * rtb_Sum1 + ID_P.DutyRatio_Offset_Value;
  if (rtb_Sum1 < 65536.0) {
    if (rtb_Sum1 >= 0.0) {
  ...
  {
    EPwm1Regs.TBPRD = (uint16_T)(ID_P.CarryFrequency_Value);
  }
  ...
  {
    EPwm2Regs.TBPRD = (uint16_T)(ID_P.CarryFrequency_Value);
  }
...
void ID_initialize(void)
{
  ...
  rtmSetErrorStatus(ID_M, (NULL));

  /* block I/O */
  (void) memset(((void *) &ID_B), 0,
                sizeof(BlockIO_ID));
